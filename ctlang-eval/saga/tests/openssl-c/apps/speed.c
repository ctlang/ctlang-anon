...
#define ITERATIONS 100000000 // hundred million
...
static const int lengths_list[] = {
    16, 64, 256, 1024, 4 * 1024, 8 * 1024, 16 * 1024
};
...
static int EVP_Update_loop(void *args)
{
    ...
    int outl, count, rc;
    int outlen = tempargs->siglen;
#ifndef SIGALRM
    int nb_iter = save_count * 4 * lengths[0] / lengths[testnum];
#endif
#if 1
    for (count = 0; COND(nb_iter); count++) {
        EVP_DecryptForTiming(tempargs->ctx, buf, &outlen, buf, outlen);
    }
#else
    if (decrypt) {
        for (count = 0; COND(nb_iter); count++) {
        ...
        for (testnum = 0; testnum < size_num; testnum++) {
            if (evp_cipher) {
                ...
                for (k = 0; k < loopargs_len; k++) {
                    ...
                    unsigned char *aad = loopargs[k].buf2;
                    unsigned char *mac_secret = loopargs[k].buf2 + 13;
                    unsigned char *buf = loopargs[k].buf;
                    RAND_bytes(iv, 16);
                    ...
                    aad[11] = dolen >> 8;
                    aad[12] = dolen;
                    RAND_bytes(buf, dolen);
                    int r;
                    r = EVP_EncryptInit_ex(loopargs[k].ctx, evp_cipher, NULL, key32, iv);
                    r = EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_AEAD_SET_MAC_KEY,
                            20, mac_secret);
                    int pad = EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_AEAD_TLS1_AAD,
                            EVP_AEAD_TLS1_AAD_LEN, aad);
                    int outlen = 0, maclen = 0;
                    int nb_iter = save_count * 4 * lengths[0] / lengths[testnum];
                    r = EVP_EncryptUpdate(loopargs[k].ctx, buf, &outlen, buf, dolen + pad);
                    EVP_EncryptFinal_ex(loopargs[k].ctx, buf + outlen, &maclen);
                    EVP_CIPHER_CTX_free(loopargs[k].ctx);

                    loopargs[k].ctx = EVP_CIPHER_CTX_new();
                    EVP_DecryptInit_ex(loopargs[k].ctx, evp_cipher, NULL, key32, iv);
