...
const OPTIONS test_options[] = {
    ...
    {"test", OPT_TEST, '-',
     "Do encryption -> decryption (only EVP)"},
...
int test_main(int argc, char **argv)
{
    ...
    prog = opt_init(argc, argv, test_options);
    ...
    if (doit[D_EVP]) {
        for (testnum = 0; testnum < SIZE_NUM; testnum++) {
            if (evp_cipher) {
                ...
                print_message(names[D_EVP], 1, lengths[testnum]);

                    int dolen = lengths[testnum];
                ...
                k = 0;
                    RAND_bytes(iv, 16);
                    RAND_bytes(mac_secret, 20);
                    RAND_bytes(aad, 9); // seq num + "type"
                    aad[9] = 3;
                    aad[10] = 0;
                    aad[11] = dolen >> 8;
                    aad[12] = dolen;

                    RAND_bytes(loopargs[k].buf, dolen);

                    if (lengths[testnum] <= 64) {
                        printf("\n");
                        for (i = 0; i < dolen; i++) {
                            printf(" %02x", loopargs[k].buf[i]);
                            if (i % 16 == 7)
                                printf(" ");
                            else if (i % 16 == 15)
                                printf("\n");
...
#define RESET "\x1b[0m"
                    ...
                    res = EVP_EncryptInit_ex(loopargs[k].ctx, evp_cipher, NULL, key32, iv);
                    if (res == 0) printf(RED" enc init fail\n");

                    EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_AEAD_SET_MAC_KEY,
                            20, mac_secret);

                int pad = EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_AEAD_TLS1_AAD,
                                          EVP_AEAD_TLS1_AAD_LEN, aad);
                    if (pad == -1) printf(RED" enc ctrl fail\n");
                    else if (pad == 0) printf(RED" enc ctrl fail\n");
                    ...
                    res = EVP_EncryptUpdate(loopargs[k].ctx, loopargs[k].buf2, &outlen, loopargs[k].buf, dolen + pad);
                    if (res == 0) printf(RED" enc upd fail\n");

                    res = EVP_EncryptFinal_ex(loopargs[k].ctx, loopargs[k].buf2 + outlen, &maclen);
                    if (res == 0) printf(RED" enc fin fail\n");

                    EVP_CIPHER_CTX_free(loopargs[k].ctx);

                    if (lengths[testnum] <= 64) {
                        for (i = 0; i < outlen; i++) {
                            printf(" %02x", loopargs[k].buf2[i]);
                            if (i % 16 == 7)
                                printf(" ");
                            else if (i % 16 == 15)
                                printf("\n");
                        }
                        for (; i < outlen + maclen; i++) {
                            printf("%c%02x", i == outlen ? '>' : ' ', loopargs[k].buf2[i]);
                            if (i % 16 == 7)
                                printf(" ");
                            else if (i % 16 == 15)
                                printf("\n");
                    ...
                    loopargs[k].ctx = EVP_CIPHER_CTX_new();
                    res = EVP_DecryptInit_ex(loopargs[k].ctx, evp_cipher, NULL, key32, iv);
                    if (res == 0) printf(RED" dec init fail\n");

                    EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_AEAD_SET_MAC_KEY,
                            20, mac_secret);

                pad = EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_AEAD_TLS1_AAD,
                                          EVP_AEAD_TLS1_AAD_LEN, aad);
                    if (pad == -1) printf(RED" dec ctrl fail\n");
                    else if (pad == 0) printf(RED" dec ctrl fail\n");
                    if (lengths[testnum] <= 64) {
                    printf(" dec pad: %d\n", pad);
                    printf(" out len: %d\n", outlen);
                    }

                    /*res = EVP_DecryptUpdate(loopargs[k].ctx, loopargs[k].buf2, &outlen, loopargs[k].buf2, outlen);
                    if (res == 0) printf(RED" dec upd fail\n");
                    printf(" out len: %d\n", outlen);

                    res = EVP_DecryptFinal_ex(loopargs[k].ctx, loopargs[k].buf2 + outlen, &maclen);
                    if (res == 0) printf(RED" dec fin fail\n");*/

                    res = EVP_Cipher(loopargs[k].ctx, loopargs[k].buf2, loopargs[k].buf2, outlen);
                    if (res == 0) printf(RED" dec cipher fail\n");
                    ...
                    outlen -= loopargs[k].buf2[outlen-1] + 1;
                    outlen -= SHA_DIGEST_LENGTH;

                    int ok = 1;
                    if (lengths[testnum] <= 64) {
                        for (i = 0; i < outlen; i++) {
                            if (loopargs[k].buf[i] != loopargs[k].buf2[i]) {
                                ok = 0;
                                printf(RED);
                            }
                            printf(" %02x", loopargs[k].buf2[i]);
                            ...
                            if (i % 16 == 7)
                                ...
                                printf("  %s\n", ok ? GREEN"OK"RESET : RED"NO"RESET);
                                ok = 1;
                            }
                        }
                        if (i % 16 != 0) {
                    ...
                    {
                        if (dolen != outlen) {
                            printf(" Length mismatch: "RED"%d"RESET" vs %d\n", outlen, dolen);
                            ok = 2;
                        }
                        int min = dolen < outlen + maclen ? dolen : outlen + maclen;
                        for (i = 0; i < min; i++) {
                            if (loopargs[k].buf[i] != loopargs[k].buf2[i]) {
                                printf(RED" Content mismatch"RESET"\n");
                                ok = 0;
                                break;
                            }
                        }
