...
let get_intrinsic_name = function
  | Memcpy sz -> "ctlang.memcpy.i" ^ (string_of_int sz)
  | Memset sz -> "ctlang.memset.i" ^ (string_of_int sz)
  | SMemset sz -> "ctlang.smemset.i" ^ (string_of_int sz)
  | Rotl sz -> "ctlang.rotl.i" ^ (string_of_int sz)
  | Rotr sz -> "ctlang.rotr.i" ^ (string_of_int sz)
  | SelectAsm sz -> "ctlang.select.asm.i" ^ (string_of_int sz)
  | SelectXor sz -> "ctlang.select.xor.i" ^ (string_of_int sz)
  | SelectSel sz -> "ctlang.select.sel.i" ^ (string_of_int sz)
  | CmovAsm sz -> "ctlang.cmov.asm.i" ^ (string_of_int sz)
  | CmovXor sz -> "ctlang.cmov.xor.i" ^ (string_of_int sz)
  | CmovSel sz -> "ctlang.cmov.sel.i" ^ (string_of_int sz)
